...
import { GetAlertsList, GetFailureAssetList, GetMaintenanceAssetList } from "@/app/services/company-admin/alerts";
...
const MaintenanceAssetList = () => {
    ...
    useEffect(() => {
        ...
        async function fetchRoles() {
            const response = await GetMaintenanceAssetList(page, pageSize);
            ...
            if (!response.has_error && response.message === "Alerts Asset Failure fetched successfully") {
    ...
    return (
        <div className='card-box bg-[#fff] border-gray-700 rounded-[18px] shadow-3xl shadow-white px-3 py-5.5'>
            <AlertHeader title={"Failed Asset List"} />
                            ...
                            {list?.length !== 0 ? (
                                <table className='table text-left border-collapse w-full text-[#111c43] border rounded-md text-[14px] leading-5 overflow-hidden'>
                                    ...
                                    <tbody className='table-row-group align-middle '>
                                        {list?.map((asset) => {
                                            return (
                                                <tr className='border border-[#f5f6f1]' key={asset.id}>
                                                    ...
                                                    <td className='p-3 align-middle text-[13px] font-medium text-[#474a54]'>
                                                        <Link
                                                            ...
                                                            href={`/company-admin/alerts/edit/${asset.alert.id}`}>
