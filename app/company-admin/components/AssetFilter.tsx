...
const AssetFilter = () => {
    ...
    async function getAssetCols() {
        try {
            const result = await clientFetch("/company/table-columns/assets", {
                method: "GET",
                headers: {
    ...
    const applyFilter = function () {
        ...
        Cookies.set("company_asset_filter", JSON.stringify(filtersArr));
        window.dispatchEvent(new Event("AssetFiltersChanged"));
    };

    const resetFilter = function () {
        setToggle(false);
        setDraftFilters([]);
        Cookies.remove("company_asset_filter");
        window.dispatchEvent(new Event("AssetFiltersChanged"));
    };
    ...
    const getCheckboxValue = (colName: string): boolean => {
        const filter = getDraftFilter(colName);
                ...
                {/* {toggle && (
                    <form className='flex flex-col gap-6'>
                        <div className='grid grid-cols-2 gap-4'>
                            {columns
                                .filter((col) => !HIDE_FIELDS.includes(col.name))
                                .map((col) =>
                                    col.type === "DATETIME" ? (
                                    ...
                                    ) : CHECKBOX_FIELDS.includes(col.name) ? (
                                        <div className='col-span-1' key={col.name}>
                                            <label>
                                                {col.name.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                                            </label>
                        ...
                        <div className='grid grid-cols-2 gap-4'>
                            {columns
                                ...
                                .map((col) => {
                                    ...
                                    if (CHECKBOX_FIELDS.includes(col.name)) {
                                        return (
                                            <div className='col-span-1' key={col.name}>
                                                <label>
                                                    <input
                                                        type='checkbox'
                                                        className='mr-2'
                                                        checked={getCheckboxValue(col.name)}
                                                        onChange={(e) =>
                                                            handleFilterChange(
                                                                col.name,
                                                                col.type,
                                                ...
                                                <div className='flex items-center gap-4'>
                                                    ...
                                                    {getTextValue(col.name) && (
                                                        <button
                                                            ...
                                                            onClick={() =>
                                                                handleFilterChange(col.name, col.type, "", 0)
                                                            }>
                                                            Clear
                                                        </button>
                                                    )}
