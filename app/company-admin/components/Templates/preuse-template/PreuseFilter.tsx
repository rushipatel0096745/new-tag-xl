...
const PreuseFilter = () => {
    ...
    async function getPreuseCols() {
        try {
            const result = await clientFetch("/company/table-columns/pre_use_check_templates", {
                method: "GET",
                headers: {
    ...
    const applyFilter = function () {
        let filtersArr = draftFilters.map((filter) => {
            if (Array.isArray(filter.text)) {
                const text = filter.text.filter(Boolean); // remove empty strings
                return {
                    ...filter,
                    text: text.length === 1 ? text[0] : text, // unwrap if single item
                };
            }
            return filter;
        });
        ...
        Cookies.set("company_preuse_filter", JSON.stringify(filtersArr));
        window.dispatchEvent(new Event("PreUseFiltersChanged"));
    };

    const resetFilter = function () {
        setToggle(false);
        setDraftFilters([]);
        Cookies.remove("company_preuse_filter");
        window.dispatchEvent(new Event("PreUseFiltersChanged"));
    };
    ...
    function handleFilterChange(colName: string, type: string, value: string, date_index: number) {
        setDraftFilters((prev) => {
            ...
            // For text input, remove filter if cleared
            ...
            const buildDateFilter = (existing: DraftFilter | undefined): DraftFilter | null => {
                ...
                // Remove filter only if both slots are empty
            ...
            // now suppose if cookies data coming in this structure how do i restore filter state with draftFilter

            const newFilter = type === "DATETIME" ? buildDateFilter(existingFilter) : buildTextFilter();

            // Remove filter if buildDateFilter returned null
            if (newFilter === null) {
                return prev.filter((fil) => fil.field !== colName);
            }
