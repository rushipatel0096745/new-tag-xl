...
const ManualFilter = () => {
    ...
    async function getManualCols() {
        try {
            const result = await clientFetch("/company/table-columns/manual_templates", {
                method: "GET",
                headers: {
    ...
    const applyFilter = function () {
        ...
        Cookies.set("company_manual_filter", JSON.stringify(filtersArr));
        window.dispatchEvent(new Event("ManualFiltersChanged"));
    };

    const resetFilter = function () {
        setToggle(false);
        setDraftFilters([]);
        Cookies.remove("company_manual_filter");
        window.dispatchEvent(new Event("ManualFiltersChanged"));
    };
