...
const CompanyList = ({ companyList }: { companyList: Company[] }) => {
    const [list, setList] = useState<Company[]>(companyList);
    ...
    useEffect(() => {
        ...
        async function fetchRoles() {
            const cookieFilters = Cookies.get("super_company_filter");
            let parsedFilters = null;
            if (cookieFilters) {
                parsedFilters = JSON.parse(cookieFilters);
            }
            console.log("parsed filters: ", parsedFilters);
            const response = await GetCompanyList(page, pageSize, parsedFilters);
            ...
            if (response.has_error && response.message === "Comapny not found") {
                setLoading(false);
                setList([]);
                setTotal(0);
                return;
            }
            if (!response.has_error && response.message === "Company fetched successfully") {
        ...
        window.addEventListener("CompanyFiltersChanged", handleFiltersChanged);

        return () => {
            window.removeEventListener("CompanyFiltersChanged", handleFiltersChanged);
        };
    }, [page, pageSize]);
    ...
    async function loginToCompany(id: number) {
        const result = await LoginToCompany(id);
        if (!result.success) {
            setPermError("Permission Denied");
        }
        if (result.success) {
            setPermError("");
            window.open("/company-admin", "_blank");
        }
    }
    ...
    return (
        <div className='card-box bg-[#fff] border-gray-700 rounded-[18px] shadow-3xl shadow-white px-3 py-5.5'>
            <div className='card-box_head border-b border-b-[#ededed] px-4 py-5.5 flex justify-between items-center'>
                <h3 className='h3 text-[18px] font-semibold leading-6'>Company List</h3>
                ...
                <div className='actions-btn flex gap-2 items-center'>
                    {userRole?.includes("create") && (
                        <Link
                            ...
                            <span className='button-label text-[#1a1a1a] capitalize ml-2'>Add Company</span>
                        </Link>
                    )}
                </div>
            </div>
                                ...
                                <table className='table text-left border-collapse w-full text-[#111c43] border rounded-md text-[14px] leading-5 overflow-hidden'>
                                    <thead className='bg-[#f5f6fa] table-header-group align-middle'>
                                        <tr className='table-row border-1 border-solid border-[#f5f6f1]'>
                                            <th className='p-2 font-medium'>Company ID</th>
                                            <th className='p-2 font-medium'>Company Name</th>
                                            <th className='p-2 font-medium'>Address</th>
                                            <th className='p-2 font-medium'>Status</th>
                                            <th className='p-2 font-medium'>Asset Limit</th>

                                            <th className='p-2 font-medium'>Subscription Validity</th>
                                        ...
                                        {list?.map((comp) => {
                                            return (
                                                <tr
                                                    ...
                                                    <td className='text-[13px] p-2 font-medium text-[#474a54]'>
                                                        {comp.country}, {comp.city}, {comp.state}, {comp.postal_code}
                                                    </td>
                                                    <td className='text-[13px] p-2 font-medium text-[#474a54]'>
                                                        {comp.status === true ? (
                                                    ...
                                                    <td className='text-[13px] p-2 font-medium text-[#474a54]'>
                                                        {comp.allowed_asset_limit}
                                                    </td>
                                                    <td className='text-[13px] p-2 font-medium text-[#474a54]'>
                                                        {comp.subscription_validity}
                                                    </td>
                                                            ...
                                                            <div className='actions-btn flex gap-2 items-center'>
                                                                {userRole.includes("update") && (
                                                                    <Link
                                                                        ...
                                                                        href={`/super-admin/company/edit/${comp.id}`}>
                                                                        <span className='icon-circle'>
                                                                            <svg
                                                                                xmlns='http://www.w3.org/2000/svg'
                                                                                width={16}
                                                                ...
                                                                {userRole.includes("login-to-company") && (
                                                                    <button
                                                                        onClick={() => loginToCompany(comp.id)}
                                                                        className='icon-button delete inline-flex items-center justify-center cursor-pointer p-0 decoration-0'
                                                                        type='button'>
                                                                        <span className='icon-circle'>
                                                                            <svg
                                                                                ...
                                                                                fill='none'>
                                                                                <path
